...
class LuciferDonghuaScraper(BaseScraper):
    def __init__(self, default_server_idx: int = 0):
        super().__init__("luciferdonghua", "https://luciferdonghua.in", default_server_idx)
    ...
    async def get_latest(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching RSS for LuciferDonghua: {e}")

        if not posts:
            html = await self.fetch_html(self.base_url)
            if html:
                parser = HTMLParser(html)
                for a in parser.css(".post-item a, .post-title a, article h2 a"):
    ...
    async def get_post(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        parser = HTMLParser(html)
        title_elem = parser.css_first("h1.post-title, h1.entry-title, h1")
    ...
    def get_description(self, parser: HTMLParser) -> str:
        ...
        content = parser.css_first(".post-content, .entry-content")
        if content:
    ...
    def get_servers(self, parser: HTMLParser) -> List[Dict[str, Any]]:
        servers = []
        for btn in parser.css(".server-button, .player-option, option"):
            name = btn.text(strip=True)
            url = btn.attributes.get("data-src") or btn.attributes.get("data-video") or btn.attributes.get("value")
    ...
    def get_download_links(self, parser: HTMLParser) -> List[Dict[str, str]]:
        downloads = []
        for a in parser.css("a[href*='mega.nz'], a[href*='gdrive'], a[href*='drive.google.com'], .dl-btn"):
            url = a.attributes.get("href")
