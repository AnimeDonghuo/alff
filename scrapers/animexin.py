...
class AnimeXinScraper(BaseScraper):
    def __init__(self, default_server_idx: int = 0):
        super().__init__("animexin", "https://animexin.dev", default_server_idx)
    ...
    async def get_latest(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching RSS for AnimeXin: {e}")

        if not posts:
            html = await self.fetch_html(self.base_url)
            if html:
                parser = HTMLParser(html)
                for a in parser.css(".post-item a, .post-title a, article a"):
