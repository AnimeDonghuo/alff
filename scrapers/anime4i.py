...
class Anime4iScraper(BaseScraper):
    def __init__(self, default_server_idx: int = 0):
        super().__init__("anime4i", "https://anime4i.com", default_server_idx)
    ...
    async def get_latest(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching RSS for Anime4i: {e}")

        if not posts:
            html = await self.fetch_html(self.base_url)
            if html:
                parser = HTMLParser(html)
                for a in parser.css(".post-item a, .entry-title a, article a"):
    ...
    def get_servers(self, parser: HTMLParser) -> List[Dict[str, Any]]:
        servers = []
        for btn in parser.css(".player-option, .server-option, option"):
    ...
    def get_download_links(self, parser: HTMLParser) -> List[Dict[str, str]]:
        downloads = []
        for a in parser.css("a[href*='mega.nz'], a[href*='gdrive'], .download-link"):
            url = a.attributes.get("href")
