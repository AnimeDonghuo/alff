...
class AnimeCubeScraper(BaseScraper):
    def __init__(self, default_server_idx: int = 0):
        super().__init__("animecube", "https://animecube.live", default_server_idx)
    ...
    async def get_latest(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching RSS for AnimeCube: {e}")
    ...
    def get_thumbnail(self, parser: HTMLParser) -> Optional[str]:
        ...
        img = parser.css_first(".featured-image img, article img, .post-thumbnail img")
        if img:
    ...
    def get_description(self, parser: HTMLParser) -> str:
        ...
        content = parser.css_first(".post-entry, .entry-content")
        if content:
    ...
    def get_servers(self, parser: HTMLParser) -> List[Dict[str, Any]]:
        servers = []
        for opt in parser.css(".server-option, .player-option, option"):
            name = opt.text(strip=True)
            url = opt.attributes.get("data-src") or opt.attributes.get("data-video") or opt.attributes.get("value")
    ...
    def get_download_links(self, parser: HTMLParser) -> List[Dict[str, str]]:
        downloads = []
        for a in parser.css("a[href*='mega.nz'], a[href*='drive.google'], .download-button"):
            url = a.attributes.get("href")
